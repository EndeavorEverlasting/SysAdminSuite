...
- **Guide-to-launcher invariant:** any request to guide a technician through an executable Windows field use case must first create or strengthen a tracked repository-owned `.cmd` launcher for that use case. Prose guidance, shell snippets, GUI buttons, dashboard actions, menus, QR wrappers, and tutorials are incomplete until they delegate to that launcher or the same proven runtime contract. The launcher must work from any working directory and any Windows username by resolving machine-neutral canonical runtime/state such as `C:\SASAL`, `%ProgramData%\SysAdminSuite`, or another registered machine-local authority instead of named-user Desktop, OneDrive, profile, or checkout paths. Only after the CMD contract is implemented and validated may guide text or GUI surfaces be added as thin wrappers.
...
- Before executing or handing any repository-owned command or launcher to an operator, prove refreshed remote truth and canonical-development-checkout currentness, and prove that the tree which will execute the command is current at the selected refreshed commit. An explicit request to run or use SysAdminSuite authorizes only the configured Git-remote freshness fetch and, when the canonical default-branch checkout is clean, owned, healthy, and strictly behind, `git pull --ff-only <intended_remote> <intended_remote_ref>` followed by a `git rev-parse HEAD` equality check against the selected refreshed commit before the product command; when the workstation itself must perform that update, path resolution, fetch, bounded pull, post-pull equality proof, entrypoint verification, and product invocation must remain one atomic handoff. A missing, dirty, diverged, unhealthy, or unproved checkout must fail closed through `harness/workflows/repository-freshness-before-launch.yaml` instead of issuing a stale installed `sas` or repo-relative command.
- **Bounded sealed-runtime protected-window exception:** refreshed provider truth is still mandatory before handoff, but an already-authenticated protected-network AutoLogon window does not have to perform workstation Git or leave the protected network when the coordinator selects an explicit immutable deployment floor from that refreshed truth and proves it is an ancestor of the refreshed default head, contains every currently required safety/capability fix for the requested lane, and is not revoked by a current contract. The workstation must then prove its sealed runtime's exact `prepared_commit`, SHA-256 tracked-file seal, local-filesystem-only/no-remotes posture, protected bootstrap, and required capability against that selected floor before mutation. This exception is a bounded floor proof, not a claim that the sealed runtime equals the latest default-branch head. If any floor or seal proof is missing, fail closed without changing the operator's current VPN/hardwire/WAB posture; refresh later through the normal Guest/Internet path.
