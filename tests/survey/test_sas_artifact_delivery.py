...
class ArtifactDeliveryTests(unittest.TestCase):
    ...
    def test_workstation_template_validates_clean_rows(self):
        with tempfile.TemporaryDirectory() as td:
            output, errors, _ = self.validate_fixture("workstation_source_clean_expected.csv", "workstation-source", Path(td))
    ...
    def test_missing_identifiers_produce_validation_error(self):
        with tempfile.TemporaryDirectory() as td:
            output, errors, _ = self.validate_fixture("workstation_source_dirty.csv", "workstation-source", Path(td), expected=1)
    ...
    def test_mac_normalization_works(self):
        with tempfile.TemporaryDirectory() as td:
            output, _, _ = self.validate_fixture("workstation_source_dirty.csv", "workstation-source", Path(td), expected=1)
    ...
    def test_duplicate_serial_prefix_creates_warning(self):
        with tempfile.TemporaryDirectory() as td:
            _, _, warnings = self.validate_fixture("serial_prefixes_sample.csv", "serial-prefixes", Path(td))
    ...
    def test_placeholder_serial_prefix_fails_in_production_mode(self):
        with tempfile.TemporaryDirectory() as td:
            _, errors, _ = self.validate_fixture("serial_prefixes_sample.csv", "serial-prefixes", Path(td), expected=1, extra=["--production-mode"])
    ...
    def test_review_queue_severity_rules_work(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            severities = {}
            for row in rows:
                severities.setdefault(row["IssueType"], set()).add(row["Severity"])
            self.assertIn("critical", severities["Serial prefix conflict"])
            self.assertIn("critical", severities["MAC conflict"])
            self.assertIn("critical", severities["Duplicate serial"])
            self.assertIn("medium", severities["Surveyed unreachable"])
            self.assertIn("medium", severities["Hostname/IP missing"])
            self.assertIn("medium", severities["Low confidence"])
    ...
    def test_package_excludes_raw_nmap_output_unless_include_raw_is_passed(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.run_cmd(base_args + ["--output-dir", str(tempdir / "delivery"), "--package-name", "sample_delivery"])
            package_dir = next((tempdir / "delivery").glob("sample_delivery_*"))
            ...
            self.run_cmd(base_args + ["--output-dir", str(tempdir / "delivery2"), "--package-name", "sample_delivery", "--include-raw"])
