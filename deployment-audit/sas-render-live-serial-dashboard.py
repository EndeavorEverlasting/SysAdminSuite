...
CLASS_DESCRIPTIONS = {
    ...
    "reachable_no_serial": "Target responds, but serial/MAC evidence was not collected. Use approved identity transport or Vision.",
    "unreachable_mark_off": "Target did not respond. Mark off and route to AD/Vision before treating it as a deployment error.",
...
def render_table(rows: list[dict[str, str]], table_id: str, title: str, description: str) -> str:
    ...
    <section class="panel">
      ...
      <div class="table-wrap">
        <table id="{esc(table_id)}"><thead><tr>{header}</tr></thead><tbody>{''.join(body)}</tbody></table>
      </div>
    </section>
    """
...
def render_dashboard(rows: list[dict[str, str]], source: Path) -> str:
    ...
    for classification in CLASS_ORDER:
        ...
        category_panels.append(render_table(subset, f"table_{classification}", CLASS_LABELS[classification], CLASS_DESCRIPTIONS[classification]))
    ...
    follow_items = "".join(f"<li><span>{esc(name)}</span><strong>{count}</strong></li>" for name, count in follow_counts.most_common())
    class_cards = "".join(f"""
...
<style>
:root {{ --bg:#050816; --panel:#0f172a; --line:#334155; --text:#e5e7eb; --muted:#94a3b8; --cyan:#22d3ee; --blue:#60a5fa; --green:#22c55e; --yellow:#facc15; --red:#f87171; }}
...
h1 {{ margin:0; font-size:30px; }} header p {{ margin:8px 0 0; color:var(--muted); }} main {{ padding:24px 34px 70px; }}
...
.metric span {{ display:block; color:var(--muted); text-transform:uppercase; font-size:12px; letter-spacing:.09em; }} .metric strong {{ display:block; font-size:34px; margin:8px 0 6px; }} .metric em {{ color:var(--muted); font-style:normal; font-size:13px; line-height:1.35; }}
.metric.good {{ border-color:rgba(34,197,94,.55); box-shadow:0 0 28px rgba(34,197,94,.12); }} .metric.warn {{ border-color:rgba(250,204,21,.55); box-shadow:0 0 28px rgba(250,204,21,.12); }} .metric.off {{ border-color:rgba(148,163,184,.42); }} .metric.bad {{ border-color:rgba(248,113,113,.6); box-shadow:0 0 30px rgba(248,113,113,.14); }} .metric.info {{ border-color:rgba(34,211,238,.5); box-shadow:0 0 28px rgba(34,211,238,.12); }}
.dashboard-grid {{ display:grid; grid-template-columns:minmax(260px,1fr) minmax(260px,1fr); gap:16px; margin-bottom:18px; }}
.panel {{ background:rgba(15,23,42,.86); border:1px solid rgba(96,165,250,.26); border-radius:20px; padding:18px; box-shadow:0 0 36px rgba(2,132,199,.09); margin-bottom:18px; }} .panel-head {{ display:flex; justify-content:space-between; gap:18px; align-items:flex-start; margin-bottom:14px; }} .panel h2 {{ margin:0 0 6px; font-size:21px; }} .panel p {{ margin:0; color:var(--muted); max-width:960px; }}
...
.status-label.good,.good-pill {{ border-color:rgba(34,197,94,.65); color:#bbf7d0; box-shadow:0 0 14px rgba(34,197,94,.16); }} .status-label.warn {{ border-color:rgba(250,204,21,.65); color:#fef3c7; }} .status-label.off {{ border-color:rgba(148,163,184,.5); color:#cbd5e1; }} .status-label.info {{ border-color:rgba(34,211,238,.65); color:#cffafe; }} .status-label.bad {{ border-color:rgba(248,113,113,.7); color:#fecaca; }} .muted-pill {{ color:var(--muted); }}
.follow-list {{ list-style:none; padding:0; margin:0; }} .follow-list li {{ display:flex; justify-content:space-between; border-bottom:1px solid rgba(51,65,85,.7); padding:10px 0; }} .follow-list span,.muted {{ color:var(--muted); }}
.filter {{ width:100%; margin:0 0 12px; padding:11px 13px; border-radius:12px; border:1px solid rgba(96,165,250,.28); background:#020617; color:var(--text); outline:none; }} .filter:focus {{ border-color:var(--cyan); box-shadow:0 0 0 3px rgba(34,211,238,.14); }}
.table-wrap {{ overflow:auto; max-height:580px; border:1px solid rgba(51,65,85,.8); border-radius:14px; }} table {{ width:100%; border-collapse:collapse; min-width:1500px; }} th,td {{ padding:9px 10px; border-bottom:1px solid rgba(51,65,85,.75); text-align:left; vertical-align:top; font-size:13px; }} th {{ position:sticky; top:0; background:#020617; z-index:2; color:#bfdbfe; }}
tr.good td {{ background:rgba(34,197,94,.08); }} tr.warn td {{ background:rgba(250,204,21,.08); }} tr.off td {{ background:rgba(148,163,184,.06); }} tr.info td {{ background:rgba(34,211,238,.07); }} tr.bad td {{ background:rgba(248,113,113,.1); }}
summary {{ cursor:pointer; color:#bfdbfe; }} pre {{ white-space:pre-wrap; overflow:auto; background:#020617; border:1px solid rgba(96,165,250,.24); border-radius:14px; padding:14px; color:#cbd5e1; }}
@media (max-width:900px) {{ .dashboard-grid {{ grid-template-columns:1fr; }} header,main {{ padding-left:18px; padding-right:18px; }} }}
</style>
</head>
<body>
<header><h1>SysAdminSuite Live Serial Probe Dashboard</h1><p>Generated {esc(dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))} from {esc(source)}</p></header>
<main>
...
<section class="metrics">{class_cards}</section>
<section class="dashboard-grid"><article class="panel"><div class="panel-head"><div><h2>Follow-Up Routing</h2><p>Where each row should go next when live probe evidence is insufficient.</p></div></div><ul class="follow-list">{follow_items}</ul></article><article class="panel"><div class="panel-head"><div><h2>Operating Rule</h2><p>Unreachable does not mean duplicate error. It means mark off and route to AD or Vision until located.</p></div></div><p class="muted">Use live confirmed rows to populate missing serials and MACs. Use manual review only where observed values conflict with tracker values.</p></article></section>
{''.join(category_panels)}
<details class="panel"><summary>Dashboard Summary JSON</summary><pre>{esc(dashboard_json)}</pre></details>
</main>
<script>for (const input of document.querySelectorAll('.filter')) {{ input.addEventListener('input', () => {{ const table = document.getElementById(input.dataset.table); if (!table) return; const needle = input.value.toLowerCase(); for (const row of table.querySelectorAll('tbody tr')) {{ row.style.display = row.innerText.toLowerCase().includes(needle) ? '' : 'none'; }} }}); }}</script>
</body></html>"""
