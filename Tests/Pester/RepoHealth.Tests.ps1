...
Describe 'UTF-8 BOM fixing' {
    ...
    It 'Does not double-BOM a file that already has one' {
        ...
        if (-not $hasBom) { [System.IO.File]::WriteAllBytes($f, ($bom + $bytes)) }
...
Describe 'Repo-wide BOM compliance' {
    It 'Can enumerate .ps1 files and detect missing UTF-8 BOM without crashing' {
        $repoRoot = Split-Path -Parent (Split-Path -Parent $PSScriptRoot)
        $files = @(Get-ChildItem -Path $repoRoot -Recurse -Filter '*.ps1' -File -ErrorAction SilentlyContinue |
            Where-Object { $_.FullName -notmatch '[\\/](\.git|node_modules|__pycache__|Output|Archive|\.vs|bin|obj|dist)([\\/]|$)' })
        ...
        $missing.Count | Should -BeGreaterOrEqual 0
    }
}
...
'@
    ...
    It 'No repo scripts use unescaped dollar signs in Write-Host string literals that would fail' {
        ...
        $knownPS7 = @(
            ...
            'Config\Fetch-Installers.ps1',
            'Config\GoLiveTools.ps1',
            'Config\archive\Inventory-Software.v1.0.0-legacy.ps1',
            'Mapping\Workers\Map-MachineWide.v5Compat.ps1',
            'mapping\Archive\remote_printer-map_startup_fixed.ps1',
            'mapping\Archive\jackson heights\Publish-EL082-Pack-Sandbox.ps1',
            'mapping\Archive\jackson heights\Publish-EL082-Pack.ps1'
        )
        $parseIssues = @()
        $files = Get-ChildItem -Path $repoRoot -Recurse -Filter '*.ps1' -File -ErrorAction SilentlyContinue |
            Where-Object { $_.FullName -notmatch '[\\/](\.git|node_modules|Output|Archive|archive|dist|_)([\\/]|$)' }
        foreach ($f in $files) {
            $rel = $f.FullName.Replace($repoRoot,'').TrimStart('\','/')
            if ($knownPS7 -contains $rel) { continue }
            $tokens = $null; $errors = $null
            [void][System.Management.Automation.Language.Parser]::ParseFile($f.FullName, [ref]$tokens, [ref]$errors)
            if ($errors.Count -gt 0) { $parseIssues += $rel }
        }
...
Describe 'Test-ScriptHealth.ps1 script contracts' {
    BeforeAll { $script:healthContent = Get-Content -Path $script:testHealthScript -Raw }
...
Describe 'Add-Utf8Bom.ps1 script contracts' {
    BeforeAll { $script:bomContent = Get-Content -Path $script:addBomScript -Raw }
...
Describe 'Invoke-RepoFileHealth.ps1 script contracts' {
    BeforeAll { $script:repoContent = Get-Content -Path $script:repoHealthScript -Raw }
