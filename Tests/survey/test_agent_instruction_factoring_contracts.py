...
REQUIRED_CAPABILITIES = {
    ...
    "proof-and-checkpointing.md",
    "language-runtime-selection.md",
    "mutation-and-evidence-boundaries.md",
    "field-command-design.md",
}
...
def test_agents_is_compact_router() -> None:
    ...
    assert "Progressive disclosure is a repository requirement" in text
    ...
    for skill_name in REQUIRED_SKILLS:
        ...
        assert expected in text, f"AGENTS.md does not route to {expected}"
...
def test_required_skills_compose_capabilities() -> None:
    ...
    for skill_name in REQUIRED_SKILLS:
        ...
        links = CAPABILITY_LINK.findall(text)
        assert links, f"{skill_name} does not reference a capability"
        for filename in links:
            assert (CAPABILITY_ROOT / filename).is_file(), f"{skill_name} references missing capability {filename}"
            referenced.add(filename)

    assert REQUIRED_CAPABILITIES <= referenced, (
        "required capabilities are orphaned: " + ", ".join(sorted(REQUIRED_CAPABILITIES - referenced))
    )
...
def test_instruction_sources_do_not_reintroduce_language_conflict() -> None:
    paths = [AGENTS, CLAUDE]
    paths.extend(SKILL_ROOT / name / "SKILL.md" for name in REQUIRED_SKILLS)
    paths.extend(CAPABILITY_ROOT / name for name in REQUIRED_CAPABILITIES)
...
def test_claude_front_door_uses_progressive_disclosure() -> None:
    ...
    assert ".claude/capabilities/README.md" in text
...
def main() -> None:
    tests = [
        ...
        test_required_skills_compose_capabilities,
