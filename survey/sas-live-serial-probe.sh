...
Expected runtime:
  Offline mode requires Bash plus python3.
  Live probing requires Bash on Windows, usually Git Bash or MSYS2 Bash.
...
if [[ -n "$IDENTITY_CSV" ]]; then
  [[ -f "$IDENTITY_CSV" ]] || fail "Identity CSV not found: $IDENTITY_CSV"
  log "Offline identity CSV mode enabled. Live Windows probe commands will not be required."
else
  has_cmd cmd.exe || fail "cmd.exe is required for live probing. Expected Bash-on-Windows runtime."
  has_cmd ping.exe || fail "ping.exe is required for live probing. Expected Bash-on-Windows runtime."
  has_cmd nslookup.exe || fail "nslookup.exe is required for live probing. Expected Bash-on-Windows runtime."
fi
