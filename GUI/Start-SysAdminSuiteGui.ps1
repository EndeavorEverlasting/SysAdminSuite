...
$script:TutorialTracks = [ordered]@{
  ...
  'DeployShortcuts' = @{
    ...
    Steps = @(
      @{ Title = 'Deploy Shortcuts: Overview'; Highlights = @('runTab','txtRunTargets'); Body = "EnvSetup\\Deploy-Shortcuts.ps1 copies shortcut files (.lnk) to the Public Desktop on remote workstations.`n`nFeatures:`n  - Optional Deploy-Shortcuts.config.psd1 + env vars for source path and targets (no script edits)`n  - Includes Nuance, Welcome to Cerner, and Microsoft Teams source patterns`n  - Authenticates via net.exe (handles 1219/3775 multi-connection errors)`n  - Detects bad passwords (1326) and re-prompts once`n  - Never overwrites existing files (logs EXISTS_DIFFERENT or UPTODATE)`n  - Supports -WhatIf for dry runs`n  - Writes structured CSV + human-readable log + transcript`n  - Taskbar pins need policy or layout (see EnvSetup\\README-TaskbarAndTeamsPins.md)" }
  ...
  'QRTasks' = @{
    ...
    Steps = @(
      ...
      @{ Title = 'QR Tasks: See Available Tasks'; Highlights = @('qrTab','cmbQRCategory'); Body = "Open the QR Generator tab.`n`nThe picker lists every registered QR task from Invoke-TechTask.ps1:`n  RAMProfile, ModelInfo, NetworkInfo, Serials, NeuronTrace, WinOptionalFeatures, PowerComfort, PowerComfortRevert`n`nSelect a task to preview the exact launch string before generating a QR.`n`nMachine Info > QR Task Runner still lists tasks when Targets is empty." }
      @{ Title = 'QR Tasks: Generate a Task QR'; Highlights = @('qrTab','txtQRPayloadPreview','btnQRGenerate'); Body = "Pick a task such as QR Task: RAMProfile.`n`nReview the read-only payload preview, then click Generate QR.`n`nThe GUI writes a .txt payload and .png image under GetInfo\Output\QRGenerator\ and shows QR generated locally. Scan to paste selected payload." }
      @{ Title = 'QR Tasks: Large Scannable Display'; Highlights = @('qrTab','btnQRShowLarge'); Body = "After generating, click Show Large QR.`n`nThe modal shows a high-resolution code plus Copy Payload and Save PNG.`n`nUse this when technicians need to scan from the screen instead of a printed label." }
      @{ Title = 'QR Tasks: Field Deployment'; Highlights = @('qrTab','cmbQRCategory','btnQRGenerate'); Body = "To deploy for field use:`n`n1. Copy the QRTasks folder to a network share`n2. In QR Generator, pick the task and edit the preview if the path must be UNC`n3. Generate QR and print the .png label`n4. Techs scan with any barcode scanner app`n`nPreferred launch form:`n  powershell.exe -NoProfile -File \\SERVER\Scripts\QRTasks\Invoke-TechTask.ps1 -Task RAMProfile`n`nKeep QR strings under 120 chars for reliable scanning." }
      @{ Title = 'QR Tasks: Adding New Tasks'; Highlights = @('qrTab','cmbQRCategory'); Body = "To add a custom task:`n`n1. Write a script in QRTasks\ that queries locally and saves output to GetInfo\Output\QRTasks`n`n2. Register it in Invoke-TechTask.ps1:`n   `$TaskMap['MyNewTask'] = 'Get-MyNewThing.ps1'`n`n3. Restart the GUI or reopen QR Generator — the new task appears in the picker`n`n4. Generate a QR from the QR Generator tab`n`nPress the Menu button below to try another tutorial." }
    )
  }
  'UtilitiesOverview' = @{
