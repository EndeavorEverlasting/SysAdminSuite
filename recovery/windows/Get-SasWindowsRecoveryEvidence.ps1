...
param(
    ...
    [string]$ExpectedBackupLabel,
    [string]$ExpectedBackupBusType,
    [string]$ExpectedBackupDiskModel,
    [string]$OutputPath,
...
foreach ($volume in (Get-Volume | Where-Object { $_.DriveLetter } | Sort-Object DriveLetter)) {
    ...
    try {
        ...
        $storageMappings += [pscustomobject]@{
            drive = $drive; disk_number = $disk.Number; disk_model = $disk.FriendlyName
            disk_serial = if ($IncludeSerials) { "$($disk.SerialNumber)".Trim() } else { $null }
            bus_type = "$($disk.BusType)"; disk_size_bytes = [int64]$disk.Size
            partition_number = $partition.PartitionNumber; partition_size_bytes = [int64]$partition.Size
            filesystem = "$($volume.FileSystem)"; label = "$($volume.FileSystemLabel)"
            free_bytes = [int64]$volume.SizeRemaining; volume_health = "$($volume.HealthStatus)"
        }
    }
    catch {
        $storageMappings += [pscustomobject]@{
            drive = $drive; disk_number = $null; disk_model = $null; disk_serial = $null; bus_type = $null
            disk_size_bytes = $null; partition_number = $null; partition_size_bytes = $null
            filesystem = "$($volume.FileSystem)"; label = "$($volume.FileSystemLabel)"
            free_bytes = [int64]$volume.SizeRemaining; volume_health = "$($volume.HealthStatus)"
        }
    }
...
foreach ($disk in (Get-PhysicalDisk | Sort-Object DeviceId)) {
    ...
    $physicalDisks += [pscustomobject]@{
        device_id = "$($disk.DeviceId)"; model = "$($disk.FriendlyName)"
        serial = if ($IncludeSerials) { "$($disk.SerialNumber)".Trim() } else { $null }
        media_type = "$($disk.MediaType)"; health_status = "$($disk.HealthStatus)"
        operational_status = @($disk.OperationalStatus | ForEach-Object { "$_" }); size_bytes = [int64]$disk.Size
        reliability = if ($reliability) {
            [pscustomobject]@{
                temperature_c = $reliability.Temperature; wear = $reliability.Wear; power_on_hours = $reliability.PowerOnHours
                read_errors_total = $reliability.ReadErrorsTotal; write_errors_total = $reliability.WriteErrorsTotal
                read_errors_uncorrected = $reliability.ReadErrorsUncorrected; write_errors_uncorrected = $reliability.WriteErrorsUncorrected
            }
        } else { $null }
    }
}

$backup = [ordered]@{ status = 'not_requested'; target = $null; versions = $null; items = $null; artifact = $null; identity = $null }
...
$ramModules = @(Get-CimInstance Win32_PhysicalMemory | ForEach-Object {
    [pscustomobject]@{
        slot = "$($_.DeviceLocator)"; manufacturer = "$($_.Manufacturer)"; part_number = "$($_.PartNumber)".Trim()
        capacity_bytes = [int64]$_.Capacity; rated_speed_mts = $_.Speed; configured_speed_mts = $_.ConfiguredClockSpeed
...
$gpu = @(Get-CimInstance Win32_VideoController | ForEach-Object { [pscustomobject]@{ name = "$($_.Name)"; driver_version = "$($_.DriverVersion)" } })
$hardware = [pscustomobject]@{
    system = [pscustomobject]@{ vendor = "$($systemProduct.Vendor)"; model = "$($systemProduct.Name)"; version = "$($systemProduct.Version)"; bios_version = "$($bios.SMBIOSBIOSVersion)"; bios_release_date = if ($bios.ReleaseDate) { $bios.ReleaseDate.ToUniversalTime().ToString('o') } else { $null } }
    cpu = [pscustomobject]@{ name = "$($cpu.Name)"; cores = $cpu.NumberOfCores; logical_processors = $cpu.NumberOfLogicalProcessors }
    memory = [pscustomobject]@{ total_visible_bytes = [int64]$os.TotalVisibleMemorySize * 1KB; free_physical_bytes = [int64]$os.FreePhysicalMemory * 1KB; modules = $ramModules }
    gpu = $gpu
}
...
$result = [pscustomobject]@{
    schema_version = '1.0'; captured_at_utc = (Get-Date).ToUniversalTime().ToString('o')
    host = [pscustomobject]@{ computer_name = $env:COMPUTERNAME; system_drive = $systemDrive; administrator = $isAdmin }
    storage = [pscustomobject]@{ drive_mappings = $storageMappings; physical_disks = $physicalDisks; pressure = $storagePressure }
    backup = [pscustomobject]$backup; windows_health = [pscustomobject]$health; hardware = $hardware
    proof = [pscustomobject]@{
        destructive_actions_performed = $false; serials_included = [bool]$IncludeSerials
