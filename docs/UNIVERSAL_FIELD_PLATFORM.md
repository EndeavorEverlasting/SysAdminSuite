...
Automatic switching is intentionally bounded to **saved WLAN profiles** that Windows already knows. The transition layer stores no Wi-Fi key material, does not inspect passwords, proves the exact destination profile, and records enough state to restore the starting WLAN after the command. Saved-profile existence is checked by asking Windows for that exact profile and using the native success/failure result; SysAdminSuite does not parse English `netsh` labels, so automatic profile proof is not tied to the display language of Windows. If an automatic transition fails, it makes a best-effort return to the starting saved WLAN and fails rather than continuing on an unproven network. A failed post-command restore also converts an otherwise successful wrapper result to failure so the operator is not told the workflow is fully complete while the workstation is stranded on the wrong network.

Network-sensitive `sas` transactions are **serialized** per controller with a machine-wide mutex from admission through product execution and restoration. A concurrent refresh/protected invocation does not wait while another command can change the shared WLAN adapter; it prints a blocked canary and returns `SAS_NETWORK_TRANSITION_BUSY`. Local-only commands do not take that network-transition lock. Invalid or incomplete protected command shapes are identified before admission and are passed to the canonical product dispatcher without automatic WLAN switching, so a usage error cannot cause a pointless network transition.
...
The installer prefers `%ProgramData%\SysAdminSuite\bin`. If Windows permissions require a current-user shim, that shim is only a command-discovery fallback; protected runtime authority still resolves independently through the local controller/runtime rules above. Before installing the network-aware shim against an existing `C:\SASAL`, the installer also verifies the controller-side network-intent dependencies (`SasFieldPlatform`, `SasOperatorSession`, `SasNetworkGuard`, and `SasBoundedNative`); an incomplete legacy runtime must be refreshed rather than producing an installed command that fails on its first canary.
...
Repository tests prove classification and routing for sanitized hardwire, WAB, VPN, guest-only, local fixed-drive, UNC, mapped-drive, sealed AutoLogon bootstrap, and stale-dispatcher-bypass fixtures. Network-intent contracts additionally prove that the canary appears before product execution, saved-WLAN transitions are paired with restoration, exact saved-profile proof does not parse localized output labels, network-sensitive transitions are serialized through restoration, invalid command shapes cannot trigger protected switching, controller dependencies are validated before shim installation, VPN/hardwire lifecycle manipulation remains fail-closed, transition code is controller-local/target-free, and restore failure cannot become a false-green command result. Printer technician-launcher contracts prove that `Map-NorthwellPrinter.cmd` remains a thin delegate to the trusted printer bootstrap, is installed beside `sas.cmd`, rejects an unqualified PATH shim, and stays aligned with repository governance/tutorial routing.
