...
The floor is a repository contract. It does not implement the pending canonical transport refactor and does not authorize network activity, target mutation, installer execution, reboot, automatic sign-in, file access, application launch, or operator acceptance.

## Frozen operation IDs

| Operation ID | Authority | Canonical output | Implementation state |
|---|---|---|---|
| `autologon.plan` | Local request validation only | `autologon_deployment_result.json` | Emitter alignment pending |
| `autologon.admin_deploy` | Explicitly authorized administrator deployment | `autologon_deployment_result.json` | Canonical transport refactor pending |
| `autologon.state_proof` | Bounded read-only state collection | `autologon_state_proof_result.json` | Emitter alignment pending |
...
| `autologon.proof_receipt_ingest` | Local source hashing and receipt rendering | `autologon_proof_receipt.json` | Producer pending |
...
Repository inspection found the following existing AutoLogon objects. They remain valid legacy inputs until an application-owned migration emits the new canonical contract.

| Existing object | Existing version | Current emitter | Frozen destination or disposition |
|---|---|---|---|
| Deployment summary | `sas-autologon-deployment-summary/v1` | `Invoke-SasAutoLogonDeployment.ps1` | Migrate to `sas-autologon-deployment-result/v1` after canonical transport integration |
...
| State delta | `sas-autologon-state-delta/v1` | `Invoke-SasAutoLogonStateDelta.ps1` | Normalize into `sas-autologon-state-proof-result/v1` |
...
| State summary | `sas-autologon-state-delta-summary/v1` | `Invoke-SasAutoLogonStateDelta.ps1` | Normalize into `sas-autologon-state-proof-result/v1` |
...
No existing public contract is renamed in place. The new versions are separate migration targets because the current outputs do not carry all classification, reason-code, privacy, reboot, sign-in, and proof-ceiling fields required by the frozen floor.
