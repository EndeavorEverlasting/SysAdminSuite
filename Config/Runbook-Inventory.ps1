∩╗┐<# Runbook-Inventory.ps1
   Inventory installed software on source/target ΓåÆ Repo\inventory\<HOST>\*.csv|html + software_superset.csv
#>
...
$paths | ForEach-Object {
  if (Test-Path $_) { Write-Host "Γ£ô $_" -ForegroundColor Green } else { Write-Warning "Missing: $_" }
}
