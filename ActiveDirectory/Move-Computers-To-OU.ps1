...
function Test-SasApprovedManagedTargetOU {
    ...
    foreach ($pattern in $forbidden) {
        if ($DistinguishedName.IndexOf($pattern, [System.StringComparison]::OrdinalIgnoreCase) -ge 0) {
            return $false
        }
    }

    $managed = @(
        'OU=Managed,OU=_Workstations',
        'OU=Managed_Shared,OU=_Workstations'
    )
    foreach ($pattern in $managed) {
        if ($DistinguishedName.IndexOf($pattern, [System.StringComparison]::OrdinalIgnoreCase) -ge 0) {
            return $true
        }
    }
