...
.DESCRIPTION
  ...
  -ChangeReference. The reference is audit/traceability evidence only; it does not validate an external ticketing
  system or grant AD privileges. More than one planned change also requires -ConfirmBatch and a raised -MaxChanges.
...
.NOTES
  ...
  The operator must obtain approval through the organization's real change/authorization process before -Apply.
  SysAdminSuite records ChangeReference for traceability but cannot independently validate that external approval.
...
if ($Apply -and [string]::IsNullOrWhiteSpace($ChangeReference)) {
    throw '-ChangeReference is required with -Apply for operator audit/traceability. It does not validate external approval.'
}
...
try {
    ...
    if ($doIO) {
        ...
        Write-SasJson -Path $planPath -InputObject ([ordered]@{
            ...
            change_reference = if ($Apply) { $ChangeReference } else { $null }
            change_reference_role = 'operator-supplied audit/traceability only; external approval not validated by SysAdminSuite'
    ...
    if ($doIO) {
        ...
        if ($moved.Count -gt 0) {
            ...
            $undoLines.Add('<# Generated by SysAdminSuite Move-Computers-To-OU.ps1. Restores only objects still in the expected target OU. Obtain rollback approval through the real organizational process before running. ChangeReference is audit evidence only. #>')
            $undoLines.Add('[CmdletBinding(SupportsShouldProcess = $true, ConfirmImpact = ''High'')]')
            $undoLines.Add('param([string]$Server, [Parameter(Mandatory=$true)][ValidateNotNullOrEmpty()][string]$ChangeReference)')
            $undoLines.Add('$ErrorActionPreference = ''Stop''')
            $undoLines.Add('Import-Module ActiveDirectory -ErrorAction Stop')
            $undoLines.Add('Write-Host "Change reference (audit only): $ChangeReference"')
    ...
    if ($Apply) {
        ...
        Write-Host "Change reference (audit only): $ChangeReference"
    } else {
