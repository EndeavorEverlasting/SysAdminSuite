...
<#
.SYNOPSIS
Remotely configures one authorized AutoLogon target through Kerberos-authenticated SMB and a passwordless S4U administrator task.

.DESCRIPTION
The current AutoLogon EXE is not qualified for LocalSystem. This lane deliberately uses a different
execution context without requiring anybody to be logged on to the target:

  current Kerberos-authenticated administrator -> SMB stage -> Task Scheduler S4U/HighestAvailable
  -> pinned local AutoLogon EXE -> SYSTEM read-only state proof -> cleanup

The S4U task stores no password and has no network access. All package bytes are staged locally on the
target before task execution. This lane does not promote or claim canonical LocalSystem compatibility.
It never reboots and never claims automatic sign-in until an attended post-reboot proof is completed.
#>
...
function Get-SasS4UOperatorIdentity {
    ...
    if ($sid -in @('S-1-5-18','S-1-5-19','S-1-5-20')) { throw 'S4U AutoLogon requires a named Windows administrator identity, not a service account.' }
...
function Request-SasS4UKerberosTicket {
    ...
    $output = @(& "$env:WINDIR\System32\klist.exe" get $Spn 2>&1 | ForEach-Object { [string]$_ })
    return [pscustomobject][ordered]@{
        spn = $Spn
        issued = ($LASTEXITCODE -eq 0)
...
function Invoke-SasS4UTask {
    ...
    try {
        ...
        $definition.RegistrationInfo.Description = 'SysAdminSuite one-time AutoLogon Kerberos S4U administrator task'
        ...
        $registered = $folder.RegisterTaskDefinition($TaskName, $definition, 6, $PrincipalName, $null, 2, $null)
        ...
        try {
            ...
            if ($lifecycle.error) { $lifecycle.error = "$($lifecycle.error); S4U task cleanup verification failed: $($_.Exception.Message)" }
            else { $lifecycle.error = "S4U task cleanup verification failed: $($_.Exception.Message)" }
        }
    }
...
if ([string]::IsNullOrWhiteSpace($OutputRoot)) {
    $OutputRoot = Join-Path $repoRoot 'survey\output\runs\autologon-kerberos-s4u'
}
