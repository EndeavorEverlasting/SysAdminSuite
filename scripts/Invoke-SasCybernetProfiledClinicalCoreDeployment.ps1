...
foreach ($required in @($catalogPath,$networkGatePath,$resolverPath,$sessionPath,$preflightPath,$recoveryPath)) {
...
if ($prior.Count -gt 0) {
    ...
    if ([string]$priorValue.status -eq 'CYBERNET_PROFILED_CLINICAL_CORE_COMPLETED') {
        ...
        [void](Set-SasOperatorSessionValues -Values @{ latest_run_id=[string]$priorValue.run_id; latest_status=[string]$priorValue.status; cleanup_status='VERIFIED'; cleanup_outstanding=$false; evidence_path=$prior[0].path; next_required_network='NONE'; next_command='sas evidence Cybernet' })
        exit 0
    }
    $cleanupProven = ($priorValue.PSObject.Properties['cleanup_succeeded'] -and [bool]$priorValue.cleanup_succeeded)
    if (-not $cleanupProven) {
        Write-Host "Prior run cleanup is unproven: $($priorValue.run_id). Running exact bounded recovery first." -ForegroundColor Yellow
        & powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File $recoveryPath -ComputerName $targetInput -RunId ([string]$priorValue.run_id)
        if ($LASTEXITCODE -ne 0) {
        ...
        foreach ($row in @($priorValue.package_results)) { if ($row -and [bool]$row.success -and $row.id) { $priorCompletedIds += [string]$row.id } }
        if ($priorValue.PSObject.Properties['completed_package_ids']) { $priorCompletedIds += @($priorValue.completed_package_ids | ForEach-Object { [string]$_ }) }
...
try {
    $result.phase='SOURCE PREFLIGHT'
    $result.checkpoint='SOURCE PREFLIGHT RUNNING'
    Save-Result
    ...
    if ($preflightFile.Count -eq 1) {
        ...
        $result.source_inventory_drift_package_count=[int]$preflight.inventory_drift_package_count
    }
    if ($preflightExit -ne 0 -or $preflightFile.Count -ne 1 -or -not [bool]$preflight.ready_for_target_staging) {
        throw "Source preflight failed before target mutation (exit $preflightExit)."
    }
...
function Get-CybernetProfile {
    ...
    $apps=@()
    foreach ($root in @('HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*','HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*')) { $apps += @(Get-ItemProperty -Path $root -ErrorAction SilentlyContinue | Where-Object { [string]$_.DisplayName -match '(?i)imprivata' } | ForEach-Object { [string]$_.DisplayName }) }
    $services=@()
    try {
        ...
        foreach ($svc in @(Get-Service -ErrorAction SilentlyContinue | Where-Object { $_.Name -match '(?i)imprivata' -or $_.DisplayName -match '(?i)imprivata' })) { $services += [pscustomobject][ordered]@{ name=$svc.Name; display_name=$svc.DisplayName; status=[string]$svc.Status; startup_type=$null } }
    }
...
try {
    ...
    foreach ($package in @($config.packages)) {
        Save-Checkpoint -Phase 'PACKAGE_STARTING' -PackageId ([string]$package.id)
        ...
        Save-Checkpoint -Phase 'PACKAGE_COMPLETED' -PackageId ([string]$package.id)
    ...
    $result.completed_package_ids=@($result.completed_package_ids | Sort-Object -Unique)
    $result.overall_success=$true
    Save-Checkpoint -Phase 'PROFILE_AFTER_CAPTURED' -PackageId $null
}
catch { $result.error=$_.Exception.Message; if (-not $result.profile_after) { try { $result.profile_after=Get-CybernetProfile } catch {} }; try { Save-Checkpoint -Phase 'ACTION_REQUIRED' -PackageId $null } catch {} }
...
'@
    ...
    $lastCompletedCount=$priorCompletedIds.Count
    while (-not (Test-Path -LiteralPath $remoteResultUnc -PathType Leaf)) {
        if (Test-Path -LiteralPath $remoteCheckpointUnc -PathType Leaf) {
            try {
                ...
                $completedNow=@($checkpoint.completed_package_ids | Sort-Object -Unique)
                if ($completedNow.Count -gt $lastCompletedCount) {
                    $lastId=[string]$completedNow[-1]
                    $position=[Array]::IndexOf($expectedIds,$lastId)+1
                    Write-Host "[$position/5] $lastId COMPLETE" -ForegroundColor Green
                    $lastCompletedCount=$completedNow.Count
                    $result.completed_package_ids=$completedNow; Save-Result
                }
            } catch {}
        }
    ...
    $result.completed_package_ids=@($workerResult.completed_package_ids | Sort-Object -Unique)
    ...
    if (($result.completed_package_ids -join '|') -ne ($expectedIds -join '|')) { throw "Completion set mismatch. Expected five packages; got $($result.completed_package_ids -join ', ')." }
    ...
    if ($result.scheduled_task_created) {
        try { $deleteOutput=@(& "$env:WINDIR\System32\schtasks.exe" /Delete /S $target /TN $taskName /F 2>&1 | ForEach-Object { [string]$_ }); $taskDeleted=($LASTEXITCODE -eq 0 -or ($deleteOutput -join ' ') -match '(?i)cannot find|does not exist|not exist') } catch { $taskDeleted=$false }
    }
    if ($result.staging_started) {
        try { if (Test-Path -LiteralPath $remoteRunUnc) { Remove-Item -LiteralPath $remoteRunUnc -Recurse -Force -ErrorAction Stop }; $runDeleted=(-not (Test-Path -LiteralPath $remoteRunUnc)) } catch { $runDeleted=$false }
    }
