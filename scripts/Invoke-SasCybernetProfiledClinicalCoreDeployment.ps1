...
param(
    ...
    [Parameter(Mandatory=$true)][ValidateSet('Cybernet')][string]$EquipmentProfile,
...
if ($EquipmentProfile -ne 'Cybernet') { throw 'This package lane is authorized only for an explicitly selected Cybernet equipment profile.' }

$repoRoot=(Resolve-Path -LiteralPath (Join-Path $PSScriptRoot '..')).Path
$catalogPath=Join-Path $repoRoot 'configs\software-packages\windows-native-package-sets.json'
$profileAuthorityPath=Join-Path $repoRoot 'Config\cybernet-client-preferences.json'
...
$readinessPath=Join-Path $repoRoot 'scripts\Invoke-SasCybernetDeploymentReadiness.ps1'
$recoveryPath=Join-Path $repoRoot 'scripts\Invoke-SasCybernetCoreRecovery.ps1'
foreach ($required in @($catalogPath,$profileAuthorityPath,$networkGatePath,$resolverPath,$sessionPath,$preflightPath,$readinessPath,$recoveryPath)) {
...
$profileAuthority=Get-Content -LiteralPath $profileAuthorityPath -Raw -Encoding UTF8 | ConvertFrom-Json
$profileLane=$profileAuthority.software.deployment_lanes.profiled_clinical_core
if ($null -eq $profileLane -or [string]$profileLane.package_set_id -ne 'cybernet-clinical-core' -or [int]$profileLane.package_count -ne 5) {
    throw 'Cybernet profile authority does not authorize the expected five-package profiled clinical-core lane.'
}
if ([string]$profileLane.autologon_behavior -ne 'observe_only_do_not_enable_disable_or_repair' -or [bool]$profileLane.automatic_reboot) {
    throw 'Cybernet profile authority does not preserve the required AutoLogon/no-reboot boundary.'
}
...
if ([string]::IsNullOrWhiteSpace($target)) { throw 'Canonical target resolver returned no FQDN.' }

$priorSession=Read-SasOperatorSession
$priorLockedTarget=[string](Get-SasObjectPropertyValue $priorSession 'target_fqdn')
$priorCleanupOutstanding=[bool](Get-SasObjectPropertyValue $priorSession 'cleanup_outstanding' $false)
if ($priorCleanupOutstanding -and $priorLockedTarget -and -not $priorLockedTarget.Equals($target,[StringComparison]::OrdinalIgnoreCase)) {
    throw "Operator session has unresolved cleanup for locked target $priorLockedTarget. Resolve that exact run before changing Cybernet targets."
}
...
Write-Host "[2/7] TARGET LOCKED: $target | PROFILE: $EquipmentProfile" -ForegroundColor Green
...
if ($prior.Count -gt 0) {
    ...
    if (-not $cleanupProven) {
        ...
        foreach ($row in @((Get-SasObjectPropertyValue $priorValue 'package_results' @()))) {
            if ($row -and [bool](Get-SasObjectPropertyValue $row 'success' $false) -and (Get-SasObjectPropertyValue $row 'id')) { $priorCompletedIds += [string](Get-SasObjectPropertyValue $row 'id') }
        }
...
$readinessRoot=Join-Path $runRoot 'transport-readiness'
...
$result=[ordered]@{
    schema_version='sas-cybernet-profiled-clinical-core/v2'
    run_id=$runId
    ...
    target_locked=$true
    equipment_profile=$EquipmentProfile
    profile_authority_path=$profileAuthorityPath
    profile_eligibility_proven=$true
    profile_eligibility_source='explicit_tracked_sas_cybernet_core_command'
    ...
    source_preflight_complete_before_target_contact=$false
    source_preflight_complete_before_target_mutation=$false
    source_preflight_path=$null
    source_inventory_drift_package_count=0
    source_manifest_path=$sourceManifestPath
    transport_readiness_path=$null
    ...
    task_name=$taskName
    remote_run_unc=$remoteRunUnc
    scheduled_task_created=$false
    scheduled_task_started=$false
    package_execution_started=$false
    completed_package_ids=@($priorCompletedIds)
    resumed_completed_package_ids=@($priorCompletedIds)
    worker_result_retrieved=$false
    cleanup_attempted=$false
    cleanup_succeeded=$false
    phase='TARGET'
    checkpoint='[2/7] TARGET LOCKED'
...
function Get-ResultPropertyBool([string]$Name) {
    if ($result.Contains($Name) -and $null -ne $result[$Name]) { return [bool]$result[$Name] }
    return $false
}
function Save-Result {
    ...
    [void](Set-SasOperatorSessionValues -Values @{
        latest_run_id=$runId
        latest_status=[string]$result.status
        latest_phase=[string]$result.phase
        latest_checkpoint=[string]$result.checkpoint
        ...
        target_contact_performed=(Get-ResultPropertyBool 'target_contact_performed')
        target_mutation_performed=(Get-ResultPropertyBool 'target_mutation_performed')
        package_execution_started=(Get-ResultPropertyBool 'package_execution_started')
        completed_package_ids=@($result.completed_package_ids)
        evidence_path=$summaryPath
...
try {
    ...
    if ($preflightExit -ne 0 -or $null -eq $preflight -or -not [bool](Get-SasObjectPropertyValue $preflight 'ready_for_target_staging' $false)) {
        throw "Source preflight failed before target contact/mutation (exit $preflightExit)."
    }
    $result.source_preflight_complete_before_target_contact=$true
    ...
    Set-Checkpoint -Phase 'TRANSPORT READINESS' -Checkpoint 'TRANSPORT READINESS RUNNING'
    $result.target_contact_performed=$true
    Save-Result
    $readiness=& $readinessPath -ComputerName $targetInput -AllowNetworkActivity -OutputRoot $readinessRoot -PassThru
    if ($LASTEXITCODE -ne 0 -or $null -eq $readiness) { throw 'Cybernet transport readiness failed before target staging.' }
    $result.transport_readiness_path=[string](Get-SasObjectPropertyValue $readiness 'result_path')
    $result.transport_classification=[string](Get-SasObjectPropertyValue $readiness 'transport_classification')
    $result.transport_ready=[bool](Get-SasObjectPropertyValue $readiness 'ready_for_deployment' $false)
    $readinessTarget=[string](Get-SasObjectPropertyValue $readiness 'resolved_fqdn')
    if (-not $result.transport_ready -or [string](Get-SasObjectPropertyValue $readiness 'status') -ne 'CYBERNET_DEPLOYMENT_READINESS_READY' -or $result.transport_classification -ne 'kerberos_smb_task_ready') {
        throw 'Cybernet Kerberos SMB plus Task Scheduler readiness was not proven before staging.'
    }
    if (-not $readinessTarget.Equals($target,[StringComparison]::OrdinalIgnoreCase)) { throw 'Readiness resolved a different target than the locked Cybernet FQDN.' }
    Set-Checkpoint -Phase 'TRANSPORT READINESS' -Checkpoint 'TRANSPORT READY: KERBEROS SMB + TASK'
    Write-Host 'TRANSPORT READY: KERBEROS SMB + TASK' -ForegroundColor Green
    ...
    foreach ($sourceRow in @($preflight.packages)) {
        ...
        $files=@($sourceRow.actual_files | ForEach-Object {
            [pscustomobject][ordered]@{ relative_path=[string]$_.relative_path; source_path=[string]$_.full_path; length=[int64]$_.length; sha256=[string]$_.sha256 }
        })
        $sourcePlan += [pscustomobject][ordered]@{
            ...
            files=$files
            inventory_drift=[bool]$sourceRow.inventory_drift
            source_selection=[string]$sourceRow.source_selection
        }
    }
    ...
    if (-not (Test-Path -LiteralPath $adminRoot -PathType Container)) { throw 'ADMIN$ access denied or unavailable after readiness proof.' }
    if (-not (Test-Path -LiteralPath $cRoot -PathType Container)) { throw 'C$ access denied or unavailable after readiness proof.' }
    ...
    foreach ($package in $sourcePlan) {
        ...
        if ($package.id -notin $priorCompletedIds) {
    ...
    $workerConfig=[ordered]@{
        ...
        checkpoint_path=$remoteCheckpointWindows
        expected_autologon_enabled=$false
        expected_package_ids=$expectedIds
        packages=$stagedPlan
        prior_completed_package_ids=@($priorCompletedIds)
    }
...
function Get-CybernetProfile {
    ...
    foreach ($root in @('HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*','HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*')) {
        $apps += @(Get-ItemProperty -Path $root -ErrorAction SilentlyContinue | Where-Object { [string]$_.DisplayName -match '(?i)imprivata' } | ForEach-Object { [string]$_.DisplayName })
    }
    $services=@()
    try {
        foreach ($svc in @(Get-CimInstance Win32_Service -ErrorAction Stop | Where-Object { $_.Name -match '(?i)imprivata' -or $_.DisplayName -match '(?i)imprivata' })) {
            $services += [pscustomobject][ordered]@{ name=[string]$svc.Name; display_name=[string]$svc.DisplayName; status=[string]$svc.State; startup_type=[string]$svc.StartMode }
        }
    }
    catch {
        foreach ($svc in @(Get-Service -ErrorAction SilentlyContinue | Where-Object { $_.Name -match '(?i)imprivata' -or $_.DisplayName -match '(?i)imprivata' })) {
            $services += [pscustomobject][ordered]@{ name=$svc.Name; display_name=$svc.DisplayName; status=[string]$svc.Status; startup_type=$null }
        }
    }
    [pscustomobject][ordered]@{
        ...
        imprivata=[ordered]@{ observed=($apps.Count -gt 0 -or $services.Count -gt 0); installed_display_names=@($apps | Sort-Object -Unique); services=$services; managed_by_this_run=$false; interpretation='observational/external state only' }
    }
}
$result=[ordered]@{
    schema_version='sas-cybernet-profiled-clinical-core-worker/v2'
    ...
    profile_after=$null
    packages=@()
    completed_package_ids=@($config.prior_completed_package_ids)
    overall_success=$false
    reboot_required=$false
    autologon_state_preserved=$null
    expected_autologon_enabled=[bool]$config.expected_autologon_enabled
    failed_package_id=$null
    error=$null
}
function Save-Checkpoint([string]$Phase,[AllowNull()][string]$PackageId) {
    [pscustomobject][ordered]@{
        schema_version='sas-cybernet-profiled-core-worker-checkpoint/v1'
        run_id=[string]$config.run_id
    ...
    } | ConvertTo-Json -Depth 20 | Set-Content -LiteralPath ([string]$config.checkpoint_path) -Encoding UTF8
}
$currentPackage=$null
try {
    ...
    Save-Checkpoint -Phase 'PROFILE_BEFORE_CAPTURED' -PackageId $null
    if ([bool]$result.profile_before.autologon.enabled -ne [bool]$config.expected_autologon_enabled) {
        throw "AutoLogon precondition mismatch. Expected enabled=$($config.expected_autologon_enabled); observed enabled=$($result.profile_before.autologon.enabled)."
    }
    foreach ($package in @($config.packages)) {
        $currentPackage=[string]$package.id
        Save-Checkpoint -Phase 'PACKAGE_STARTING' -PackageId $currentPackage
        ...
        $result.packages=@($result.packages)+@($row)
        if ($success) { $result.completed_package_ids=@($result.completed_package_ids)+@([string]$package.id) }
    ...
    $observed=@($result.completed_package_ids | Sort-Object -Unique)
    $result.completed_package_ids=@($config.expected_package_ids | Where-Object { $_ -in $observed })
    $result.overall_success=$true
    Save-Checkpoint -Phase 'PROFILE_AFTER_CAPTURED' -PackageId $null
}
catch {
    $result.failed_package_id=$currentPackage
    $result.error=$_.Exception.Message
    if (-not $result.profile_after) { try { $result.profile_after=Get-CybernetProfile } catch {} }
    try { Save-Checkpoint -Phase 'ACTION_REQUIRED' -PackageId $currentPackage } catch {}
}
...
'@
    ...
    $result.package_execution_started=$true
    Set-Checkpoint -Phase 'SYSTEM INSTALL' -Checkpoint '[5/7] SYSTEM INSTALL RUNNING'
    Write-Host '[5/7] SYSTEM INSTALL RUNNING' -ForegroundColor Green

    $deadline=(Get-Date).AddSeconds($ResultTimeoutSeconds)
    $lastCompleted=@($priorCompletedIds)
    while (-not (Test-Path -LiteralPath $remoteResultUnc -PathType Leaf)) {
        if (Test-Path -LiteralPath $remoteCheckpointUnc -PathType Leaf) {
            try {
                ...
                foreach ($newId in @($completedNow | Where-Object { $_ -notin $lastCompleted })) {
                    $position=[Array]::IndexOf($expectedIds,$newId)+1
                    Write-Host "[$position/5] $newId COMPLETE" -ForegroundColor Green
                }
                if ($completedNow.Count -ne $lastCompleted.Count) {
                    $lastCompleted=@($completedNow)
                    $result.completed_package_ids=@($completedNow)
                    Save-Result
                }
            } catch {}
        }
    ...
    $priorRows=@($priorCompletedIds | ForEach-Object { [pscustomobject][ordered]@{ id=$_; success=$true; result='preserved_prior_success_not_reexecuted' } })
    $result.package_results=@($priorRows)+@($workerResult.packages)
    ...
    Save-Result
    Write-Host "ACTION REQUIRED: $failure" -ForegroundColor Yellow
}
finally {
    ...
    if ($result.cleanup_succeeded) {
        $result.checkpoint='[7/7] CLEANUP VERIFIED'
        $result.checkpoint_history=@($result.checkpoint_history)+@([pscustomobject][ordered]@{ at_utc=(Get-Date).ToUniversalTime().ToString('o'); phase='CLEANUP'; checkpoint='[7/7] CLEANUP VERIFIED' })
        Write-Host '[7/7] CLEANUP VERIFIED' -ForegroundColor Green
    }
    if ($deploymentSucceeded -and $result.cleanup_succeeded) {
        $result.status='CYBERNET_PROFILED_CLINICAL_CORE_COMPLETED'
        $result.phase='COMPLETED'
        $result.reason=$null
    }
    elseif (-not $result.cleanup_succeeded) {
        $result.status='ACTION_REQUIRED'
        if (-not $result.reason) { $result.reason='Deployment transaction cleanup is not proven.' }
    }
    Save-Result
}
