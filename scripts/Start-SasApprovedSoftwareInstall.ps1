...
foreach ($requiredPath in @($catalogPath, $apiPath, $installScript, $targetIntakeModule)) {
...
function Invoke-SasInstallWrapper {
    ...
    if ($PlanOnly) {
        $output = @(& $installScript @params -WhatIf)
        $state.workflow_status = 'install_planned_whatif'
    }
    else {
        $output = @(& $installScript @params -AllowTargetMutation -Confirm:$false)
        $state.workflow_status = 'install_attempted'
    }

    $summary = @($output | Where-Object {
        $_ -and $_.PSObject.Properties.Name -contains 'operator_handoff_path'
    } | Select-Object -Last 1)
    if ($summary.Count -ne 1) { throw 'Canonical install wrapper did not return its summary object.' }

    $state.install_summary_path = [string]$summary[0].operator_handoff_path
