...
.DESCRIPTION
    Plan is the default and is read-only. Apply, Start, and Stop require
    -AllowMutation. The wrapper selects an explicit non-Docker WSL distribution,
    converts repository paths with wslpath, and delegates behavior to the Bash
    service without collecting or forwarding API keys.
#>
[CmdletBinding(SupportsShouldProcess = $true, ConfirmImpact = 'High')]
param(
    [ValidateSet('Plan', 'Apply', 'Start', 'Status', 'Stop', 'Validate')]
    [string]$Action = 'Plan',
...
$mutatingActions = @('Apply', 'Start', 'Stop')
