...
display/menu button once, then captures relevant Windows event-log entries created during that window.
It does not change power policy, firmware, registry, services, scheduled tasks, or logs.
...
- NO_WINDOWS_EVENT_OBSERVED means Windows did not log a related event during this run; the button may
  be firmware-only or display-OSD controlled, or may require a different observation method.

This probe never claims the physical display/menu button is disabled.
#>
param(
    [ValidateRange(5, 300)]
...
Add-Line -Lines $lines -Text 'This probe does not prove or claim that the physical display/menu button is disabled.'
Add-Line -Lines $lines -Text ''
...
if ($eventCount -gt 0) {
    ...
    foreach ($event in $events) {
        ...
        if ($event.Message) { Add-Line -Lines $lines -Text "  $($event.Message)" }
    }
}
else {
    Add-Line -Lines $lines -Text 'No matching Windows events were observed during this run.'
    Add-Line -Lines $lines -Text 'Interpretation: the physical display/menu button may be firmware-only or display-OSD controlled, or may not emit a Windows event under this observation method.'
}
...
if ($readErrors.Count -gt 0) {
    ...
    foreach ($errorText in $readErrors) { Add-Line -Lines $lines -Text "  $errorText" }
}
