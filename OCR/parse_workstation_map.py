...
def main() -> None:
    ...
    ap.add_argument("--legend-keyword", default="workstation")
    ...
    write_entities_detailed_csv(rows, args.out_csv, "WorkstationID")

    if args.out_overlay:
        overlay_rows = [(int(r["id"]), int(r["x"]), int(r["y"])) for r in rows]
        overlay = draw_overlay(bgr, overlay_rows, (0, 0, 255), "Workstations")
    ...
    write_universal_html_report(
        ...
        title="Workstation Map Parse Report",
        subtitle=args.map,
        rows=rows,
