...
PROCEDURE = ROOT / "harness/skills/harness-maintenance.md"
STATUS = ROOT / "docs/HARNESS_STATUS.md"
RENDERER = ROOT / "harness/reports/render-harness-status.py"


def load(path: Path) -> dict:
    assert path.is_file(), f"missing harness JSON authority: {path.relative_to(ROOT)}"
...
def test_validator_registry() -> None:
    ...
    required = {
        "harness-registry-integrity", "operational-harness-completeness",
        "local-harness-contracts", "repository-text-policy-staged",
        "repository-text-policy-commit", "patch-whitespace", "offline-survey-floor",
        "pester-full", "managed-tests-release", "dashboard-publish",
    }
...
def test_command_registry() -> None:
    ...
    required = {
        "harness-validate", "harness-completeness", "offline-floor", "powershell-tests",
        "managed-tests", "dashboard-build", "dashboard-launch", "cybernet-plan",
    ...
    for item in commands:
        ...
        assert item.get("mutation") in {
            "none", "repository_read_only", "repository_build_output_only",
            "local_runtime", "authorized_target_mutation",
...
def test_fresh_agent_wiring() -> None:
    ...
    for marker in (
        ...
        "harness/api/harness-artifact-registry.json", "harness/skills/harness-maintenance.md",
        "python harness/validators/validate-harness-registries.py", "git diff --check",
    ...
    for marker in (
        ...
        "harness/api/harness-validator-registry.json", "Do not place harness-only procedures under `.claude/skills/`",
    ):
        assert marker in procedure, f"harness-maintenance procedure missing: {marker}"
