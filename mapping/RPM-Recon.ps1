...
# Early breadcrumbs so you know we’re past staging and into scheduler territory
EnqRun "[${target}] HANDOFF → scheduler (TR len: $($tr.Length))"
EnqRun "[${target}] TR: $tr"
...
try {
  ...
  EnqRun "[${target}] REMOTE LOGS DIR CHECK ERROR: $remoteLogs — $($_.Exception.Message)"
}
