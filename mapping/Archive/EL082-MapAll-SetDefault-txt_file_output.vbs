' EL082-MapAll-SetDefault.vbs
' Maps ALL EL082 printers and sets default based on hostname tail (MST###).
' Safe to run repeatedly; works under standard user context.
...
' Disable "Let Windows manage my default printer" (per-user)
...
' Map all printers (PrintUI is reliable), then WSH fallback
Dim i, share
...
' Ensure default exists then set it
Dim defaultShare : defaultShare = PRINT_SERVER & "\" & desiredDefault
MapPrinter defaultShare
SetDefault defaultShare

' Tiny log for verification
Dim logp : logp = shell.ExpandEnvironmentStrings("%TEMP%") & "\EL082-PrinterMap.log"
LogLine logp, Now & " Host=" & host & " tail=" & tail & " Default=" & defaultShare
...
Sub LogLine(path, line)
  On Error Resume Next
  Dim ts: Set ts = fso.OpenTextFile(path, 8, True)
  ts.WriteLine line
  ts.Close
  On Error GoTo 0
End Sub
