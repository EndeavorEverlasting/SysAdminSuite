...
.DESCRIPTION
    ...
    Successful Map groups feed the same advisory per-user interaction history as
    quick mapping. Unmap groups do not erase history; cache data remains a suggestion
    only and never substitutes for authoritative desired-state proof.
#>
...
function Save-SasBatchInteractionHistory {
    ...
    try {
        ...
        # Cache persistence is advisory and must never alter authoritative batch success/failure.
    }
}
...
Write-Host 'WAB Wi-Fi, approved hardwire, and authenticated VPN are valid protected controller paths.' -ForegroundColor Green
...
foreach ($group in $groups) {
    ...
    try {
        ...
        if ($WhatIf) {
            ...
            if ($success -and $group.Action -eq 'Map') { Save-SasBatchInteractionHistory -Group $group }
        }
    }
    catch {
        $success = $false
